...
export const SalesHistoryView: Component = () => {
  ...
  const [voidingBusy, setVoidingBusy] = createSignal(false);

  async function confirmVoid(): Promise<void> {
    if (voidingBusy()) return;
    ...
    try {
      ...
      await voidTicketRequest(ticket.id, reason, approvalToken);
      closeVoidModal();
      beepSuccess();
      showNotice(`Venta #${ticket.number} anulada por ${voidedBy} — el stock volvió y el efectivo salió de caja`);
      void refetch();
      bumpCashRefresh();
    } catch (cause) {
      beepError();
